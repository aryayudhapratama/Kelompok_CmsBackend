...
<body class="m-0 font-sans text-base antialiased font-normal dark:bg-slate-900 leading-default bg-gray-50 text-slate-500">
        ...
        <div class="items-center block w-auto max-h-screen overflow-auto h-sidenav grow basis-full">
            <ul class="flex flex-col pl-0 mb-0">
                <li class="mt-0.5 w-full">
                    <a class="py-2.7 bg-blue-500/13 dark:text-white dark:opacity-80 text-sm ease-nav-brand my-0 mx-2 flex items-center whitespace-nowrap rounded-lg px-4 font-semibold text-slate-700 transition-colors" href="/reporter">
                ...
                <li class="mt-0.5 w-full">
                    <a class="dark:text-white dark:opacity-80 py-2.7 text-sm ease-nav-brand my-0 mx-2 flex items-center whitespace-nowrap px-4 transition-colors" href="{{ route('reporter.berita') }}">
                        <div class="mr-2 flex h-8 w-8 items-center justify-center rounded-lg bg-center stroke-0 text-center xl:p-2.5">
            ...
            <div class="flex items-center justify-between w-full px-4 py-1 mx-auto flex-wrap-inherit">
                <nav>
                    <ol class="flex flex-wrap pt-1 mr-12 bg-transparent rounded-lg sm:mr-16">
                        ...
                        <li class="text-sm pl-2 capitalize leading-normal text-slate-700 before:float-left before:pr-2 before:text-gray-600 before:content-['/']" aria-current="page">Berita</li>
                    </ol>
                    <h6 class="mb-0 font-bold capitalize dark:text-white">Daftar Berita Saya</h6>
                </nav>
            </div>
        </nav>
                                ...
                                <table class="items-center w-full mb-0 align-top border-collapse dark:border-white/40 text-slate-500">
                                    <thead class="align-bottom">
                                        <tr>
                                            ...
                                            <th class="px-6 py-3 pl-2 font-bold text-left uppercase align-middle bg-transparent border-b border-collapse shadow-none dark:border-white/40 dark:text-white text-xxs border-b-solid tracking-none whitespace-nowrap text-slate-400 opacity-70">Nama Reporter</th>
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            <td class="p-2 align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                                                <p class="mb-0 text-xs font-semibold leading-tight dark:text-white dark:opacity-80">{{ $berita->judul }}</p>
                                            </td>
                                            <td class="p-2 align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                                                <p class="mb-0 text-xs font-semibold leading-tight dark:text-white dark:opacity-80">{{ $berita->nama_reporter }}</p>
                                            </td>
                                            <td class="p-2 align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                                                <p class="mb-0 text-xs font-semibold leading-tight dark:text-white dark:opacity-80">{{ $berita->email_reporter }}</p>
                                            </td>
                                            <td class="p-2 align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                                                <p class="mb-0 text-xs font-semibold leading-tight dark:text-white dark:opacity-80">{{ $berita->created_at->format('d M Y') }}</p>
                                            </td>
                                            <td class="p-2 align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                                                <span class="text-xs font-semibold px-4 py-2 rounded-lg
                                            ...
                                            <td class="p-2 align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                                                <a href="javascript:;" class="btn-detail text-xs font-semibold leading-tight dark:text-white dark:opacity-80 text-slate-400"
    ...
    <!-- Modal Tambah -->
    <div id="addModal" class="hidden fixed inset-0 z-[1000] flex items-center justify-center bg-black/50">
        <div class="bg-white dark:bg-slate-800 rounded-lg p-6 w-11/12 md:w-1/2 shadow-2xl">
            ...
            <form action="{{ route('berita.store') }}" method="POST">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="nama_reporter" value="{{ auth()->user()->name }}" class="w-full px-3 py-2 border rounded-lg dark:bg-slate-700 dark:text-white" readonly>
                </div>
                <div class="mb-4">
                    <label class="block mb-1 text-sm dark:text-white">Email</label>
                    <input type="email" name="email_reporter" value="{{ auth()->user()->email }}" class="w-full px-3 py-2 border rounded-lg dark:bg-slate-700 dark:text-white" readonly>
                </div>
                <div class="mb-4">
                    <label class="block mb-1 text-sm dark:text-white">Judul Berita</label>
                    <input type="text" name="judul" class="w-full px-3 py-2 border rounded-lg dark:bg-slate-700 dark:text-white" required>
                </div>
                <div class="mb-4">
                    <label class="block mb-1 text-sm dark:text-white">Konten Berita</label>
                    <textarea name="konten" rows="6" class="w-full px-3 py-2 border rounded-lg dark:bg-slate-700 dark:text-white" required></textarea>
                </div>
                <div class="flex justify-end">
                    <button type="button" onclick="closeAddModal()" class="px-4 py-2 mr-2 text-sm bg-gray-300 rounded-lg dark:bg-slate-600 dark:text-white">Batal</button>
    ...
    {{-- Modal Edit --}}
    <div id="editModal" class="hidden fixed inset-0 z-[1000] flex items-center justify-center bg-black/50">
        <div class="bg-white dark:bg-slate-800 rounded-lg p-6 w-11/12 md:w-1/2 shadow-2xl">
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <input type="text" id="editJudul" class="w-full px-3 py-2 border rounded-lg bg-gray-100 dark:bg-slate-700 dark:text-white" readonly>
                </div>
                <div>
                    <label class="block mb-1 text-sm dark:text-white">Konten</label>
                    <textarea id="editKonten" rows="6" class="w-full px-3 py-2 border rounded-lg bg-gray-100 dark:bg-slate-700 dark:text-white" readonly></textarea>
                </div>
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="block mb-1 text-sm dark:text-white">Nama Reporter</label>
                        <input type="text" id="editNama" class="w-full px-3 py-2 border rounded-lg bg-gray-100 dark:bg-slate-700 dark:text-white" readonly>
                    </div>
                    <div>
                        <label class="block mb-1 text-sm dark:text-white">Email</label>
                        <input type="text" id="editEmail" class="w-full px-3 py-2 border rounded-lg bg-gray-100 dark:bg-slate-700 dark:text-white" readonly>
                    </div>
                </div>
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="block mb-1 text-sm dark:text-white">Tanggal Dibuat</label>
                        <input type="text" id="editTanggal" class="w-full px-3 py-2 border rounded-lg bg-gray-100 dark:bg-slate-700 dark:text-white" readonly>
                    </div>
                    <div>
                        <label class="block mb-1 text-sm dark:text-white">Status</label>
                        <input type="text" id="editStatus" class="w-full px-3 py-2 border rounded-lg bg-gray-100 dark:bg-slate-700 dark:text-white" readonly>
                    </div>
                </div>
            </div>
            <div class="flex justify-end mt-6">
                <button type="button" onclick="closeEditModal()" class="px-4 py-2 text-sm bg-gray-300 rounded-lg dark:bg-slate-600 dark:text-white">Tutup</button>
            </div>
        </div>
    </div>
