...
<body x-data="sidebarState()" x-init="init()" x-cloak>
  ...
  >
    ...
    <div class="overflow-y-auto h-[calc(100vh-120px)] scrollbar-hide">
      <ul class="flex flex-col px-3 py-2 space-y-1 text-sm">
        <li>
          <a href="/admin"
            class="flex items-center gap-3 px-4 py-2 rounded-lg transition-all duration-150 {{ request()->is('admin') ? 'bg-blue-50 text-blue-700 font-semibold' : 'text-gray-700 hover:bg-gray-100' }}">
        ...
        <li>
          <a href="{{ route('admin.users') }}"
            class="flex items-center gap-3 px-4 py-2 rounded-lg transition-all duration-150 {{ request()->is('admin/users*') ? 'bg-green-50 text-green-700 font-semibold' : 'text-gray-700 hover:bg-gray-100' }}">
        ...
        <li>
          <a href="{{ route('admin.file-manager.index') }}"
            class="flex items-center gap-3 px-4 py-2 rounded-lg transition-all duration-150 {{ request()->is('admin/file-manager*') ? 'bg-indigo-50 text-indigo-700 font-semibold' : 'text-gray-700 hover:bg-gray-100' }}">
  ...
  >
    ...
    >
      ...
      <form method="POST" action="{{ route('settings.update') }}" enctype="multipart/form-data" class="px-6 py-5 space-y-4 text-sm text-gray-700">
        @csrf
        @method('PUT')
